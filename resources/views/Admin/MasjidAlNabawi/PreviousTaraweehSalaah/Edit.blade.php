                ...
                <div class="col-lg-6 col-lg-offset-3 col-xs-12">
                    <div class="box box-success">
                        <div class="box-header with-border">
                            <div class="box-title">
                                <h3>{{ trans('admin.edit_previous_taraweeh_salaah') }}</h3>
                            </div>
                        </div>
                        <div class="box-body">
                            <form action="{{ route('nabawi.previous.taraweeh.salaah.data.update') }}" id="previous_taraweeh_salaah_form" class="form-horizontal"
                                ...
                                <div class="form-body">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-6">
                                            <select class="select2 form-control" name="year_id" id="year_id">
                                                <option value="">{{ trans('admin.select_year') }}</option>
                                                @foreach ($year as $yr)
                                                    <option value="{{ $yr->year_id }}"
                                                        {{ $yr->year_id == $data->year_id ? 'selected' : '' }}>
                                                        {{ $yr->year_en }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    ...
                                    <div class="form-group">
                                        <label class="control-label col-md-3">{{ trans('admin.select_surah_req') }}
                                            <span class="required" aria-required="true"> </span>
                                        </label>
                                        <div class="col-md-6">
                                            <select class="select2 form-control" name="surah_id" id="surah_id">
                                                <option value="">{{ trans('admin.select_surah_req') }}</option>
                                                @foreach ($surah as $sur)
                                                    <option value="{{ $sur->surah_id }}"
                                                        {{ $sur->surah_id == $data->surah_id ? 'selected' : '' }}>
                                                        {{ $sur->surah_en }}</option>
                                                @endforeach
                                            </select>
                                        </div>
